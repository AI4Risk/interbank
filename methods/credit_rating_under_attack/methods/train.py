...
parser.add_argument('--method', type=str, default="PSAGNN",
        choices=["PSAGNN","SAGNN"], help='method')
...
parser.add_argument('--alpha', type=float, default=0.7, help='weight of loss')
parser.add_argument('--beta', type=float, default=0.2, help='weight of label similarity')
parser.add_argument('--gamma', type=float, default=0.1, help='weight of feature similarity')
...
if args.method == "SAGNN":
    sagnn = SAGNNNet(features.shape[1], len(label_to_index), hiddim=args.hiddim, droprate=args.droprate,hidlayers=args.hidlayers, p = args.p).to(device)
    from SAGNN import Model
...
if args.method == "SAGNN":
    ...
    for (batch,testbatch) in zip(tqdm(loader),tqdm(testload)):
        i=i+1
        #since batchsize=14245, there is only one batch
        if i>1:
            break
        best_acc=0
        for epoch in tqdm(range(args.epochs)):  
            drop_all=model.fit(epoch,batch,drop_all)
            tru, pred=model.test(testbatch,drop_all)
            accuracy=accuracy_score(tru[:batchsize], pred[:batchsize])
            if accuracy>best_acc:
                best_acc=accuracy
                print("epoch {}".format(epoch))
                print(accuracy)
                print(classification_report(tru[:batchsize], pred[:batchsize],digits=5))     
    print(best_acc)
